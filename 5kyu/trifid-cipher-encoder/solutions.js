...
//   let divide = arr1
//     .map((x, y) =>
//       x.map((a, b) => a.concat(arr2[y][b])).map((a, b) => a.concat(arr3[y][b]))
//     )
...
//   }
//   return arr4
//     .map((x) => key[(x[0] - 1) * 9 + ((x[1] - 1) * 3 + x[2]) - 1])
//     .join('');
// }
...
console.log(
  trifidDecode('EPSDUCVWYM+ZLKXNBTFGORIJHAQ', 7, 'RSQLPNPQBLB'),
  'ABCDEFGHIJK'
);
console.log(
  trifidDecode('EPSDUCVWYM+ZLKXNBTFGORIJHAQ', 2, 'JJTAUBLJOEMIESXWYIKM'),
